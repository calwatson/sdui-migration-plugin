...
1. **Domains** — bounded contexts (resource + the GETs that load them). Name them from the code, not from the reference catalog/orders demo.
...
Do not rewrite the app. Do not invent a template engine. Do not copy Scute/Tortise names onto customer folders.
